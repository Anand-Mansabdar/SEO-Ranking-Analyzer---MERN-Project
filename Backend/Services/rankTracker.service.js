...
export const rankTracker = async (keyword, targetDomain) => {
  ...
  try {
    ...
    // 2. Initial Google Visits and Consent Handling
    ...
    // 3. Search Loop: Iterate upto 5 pages of google results
    ...
    for (let googlePage = 0; googlePage < 5; googlePage++) {
      ...
      // 5. Result Evaluation :- Update Global Results and Check for target match
      for (const result of pageResults) {
        result.position = allResults.length + 1;
        allResults.push(result);

        if (
          (!found && result.domain.toLowerCase().includes(cleanTarget)) ||
          cleanTarget.includes(result.domain.toLowerCase())
        ) {
    ...
    // 6. Final Operation :- Close Browser and Extract Competitors
    await browser.close();
