        ...
        {loading ? (
          ...
          <div className="space-y-3" style={{ animationDelay: "200ms" }}>
            {processedData.map((a) => (
              <div
                key={a._id}
                className="glass rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:bg-muted/50 transition-all group"
              >
                {/* Score */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${a.status === "completed" ? "bg-success/10 text-success" : a.status === "processing" ? "bg-primary/10 text-primary" : "bg-danger/10 text-danger"}`}
                    >
                      {a.status}
                    </span>
                  </div>
                  ...
                  <div className="hidden lg:grid grid-cols-4 gap-4">
                    {[
                      ...
                      <div key={c.label} className="text-center w-12">
                        <p
                          className={`text-sm font-bold ${getScoreClass(c.value)}`}
                        >
                          {c.value}
                        </p>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-8">
            <button
              ...
              disabled={page === 1}
              className="px-4 py-2 rounded-xl glass text-sm text-foreground disabled:opacity-30 hover:bg-muted/50 transition-all"
            >
              Previous
            </button>
